...
beforeAll(async () => {
    ...
    // Ambil accessToken (bukan .token)
    tokenPenjual = loginResPenjual.body.accessToken;

    const loginResAdmin = await request(app).post('/api/login').send({
        username: `AdminTest_${unique}`,
        password: 'password123'
    });
    tokenAdmin = loginResAdmin.body.accessToken;
});
...
describe('Pengujian API Penjual - Manajemen Menu', () => {
    ...
    it('AC-01: Harus berhasil menambahkan menu baru jika form lengkap', async () => {
        ...
        // Simpan menuId untuk test berikutnya
    ...
    it('AC-04: Harus berhasil menghapus menu', async () => {
        // PERBAIKAN: id_penjual dikirim di dalam body (.send) sesuai dengan validasi di menuController.js (req.body)
        const res = await request(app)
            .delete(`/api/menu/${menuId}`)
            .set('Authorization', `Bearer ${tokenPenjual}`)
            .send({ id_penjual: penjualId });
